...
# 初始化 Poe 客户端 (使用 OpenAI SDK 兼容模式)
...
if "condition" not in st.session_state:
    st.session_state.condition = "Condition 1" # 默认条件1  
...
if not st.session_state.started:
    ...
    with col1:
        pid = st.text_input("Please enter your ID number:", value=st.session_state.participant_id)
    with col2:
        condition = st.selectbox("Please select the condition:", ["Condition 1", "Condition 2", "Condition 3"])
        
    if st.button("Enter Experiment", use_container_width=True):
        if pid.strip() == "":
...
with st.sidebar:
    ...
    if is_admin_mode:
        ...
        st.write("---")
        st.caption("Testing in progress")

# --- 8. 主界面布局 ---
if st.session_state.condition == "Condition 1":
    # 条件1：上面 1/3 题目，下面 2/3 写作区
    st.markdown('<div class="section-header"><h4>Writing Task</h4></div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-header"><h4>Notes</h4></div>', unsafe_allow_html=True)
    ...
    if st.button("Finish Writing", type="primary", use_container_width=True):
        save_data = {
    ...
    st.markdown("<br>", unsafe_allow_html=True)
    st.markdown('<div class="section-header"><h4>Chat with AI</h4></div>', unsafe_allow_html=True)
    ...
    if prompt := st.chat_input("Ask AI..."):
        ...
        try:
            ...
            if st.session_state.condition == "Condition 2":
